...
function ApiRow({ name, type, desc }: { name: string; type: string; desc: string }) {
  return (
    <tr className="hover:bg-black/[0.03]">
      <td className="px-4 py-2.5 font-mono text-xs font-semibold text-[#0e0f0c]">{name}</td>
      <td className="px-4 py-2.5 text-xs text-[#454745]">{type}</td>
    ...
    <div className="space-y-6">
      <div>
        <div className="mb-3 flex items-center gap-2">
          <span className="rounded-full border border-blue-500/30 bg-blue-500/10 px-2.5 py-0.5 text-xs font-medium text-blue-700">@flupy/core</span>
        ...
        <h1 className="mb-3 text-4xl font-bold tracking-tight text-[#0e0f0c]">@flupy/core</h1>
        <p className="text-lg text-[#454745]">
          Protocol-level primitives shared by <code className="text-[#0e0f0c]">@flupy/browser</code> and{' '}
          <code className="text-[#0e0f0c]">@flupy/react</code>. Pure TypeScript — no browser APIs, no React, no wallet dependency.
        </p>
      </div>
      ...
      <p className="text-sm text-[#454745]">
        ...
        byte arrays (<code className="text-[#0e0f0c]">BytesN&lt;64&gt;</code>,{' '}
        <code className="text-[#0e0f0c]">BytesN&lt;128&gt;</code>,{' '}
        <code className="text-[#0e0f0c]">BytesN&lt;32&gt;</code>). The encoding helpers in{' '}
        <code className="text-[#0e0f0c]">@flupy/core</code> handle this conversion:
      </p>
      <div className="overflow-hidden rounded-xl border border-black/10 bg-white p-4 text-xs text-[#454745]">
