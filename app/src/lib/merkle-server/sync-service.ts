...
import { getServerTree } from './tree-cache';
...
export type SyncResult =
  | { synced: true; txHash: string; newRoot: string }
  | { skipped: 'lock held' | 'already in sync' }
...
/**
 * Core Merkle root sync logic, callable directly from any server-side
 * context in the same process:
 *
 * - The /api/admin/sync-root GET handler (external cron trigger)
 * - Fire-and-forget from /api/merkle-proof/enroll right after a new
 *   commitment is added, so a freshly enrolled user doesn't have to
 *   wait for the next scheduled cron run before their proof validates
 *   on-chain
 *
 * Calling this directly (vs. an HTTP fetch to the sync-root endpoint)
 * skips the network round-trip, the CRON_SECRET check, and the Vercel
 * Deployment Protection bypass header entirely -- all of which only
 * matter for genuinely external callers.
 *
 * Same guards as the original endpoint: distributed lock (skip if a
 * sync is already in flight) and idempotency (skip if on-chain root
 * already matches).
 */
export async function syncMerkleRoot(): Promise<SyncResult> {
  if (!RPC_URL || !CONTRACT_ID || !NETWORK_PASSPHRASE || !OPERATOR_SECRET || !DATABASE_URL) {
    return { error: 'sync_endpoint_misconfigured', message: 'One or more required env vars is missing' };
  }
  ...
  try {
    ...
    return { synced: true, txHash, newRoot: cacheRootHex };
  } catch (err) {
    console.error('[syncMerkleRoot] error:', err);
    return {
      error: 'sync_failed',
