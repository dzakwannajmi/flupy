...
async function getZeroHashes() {
  ...
  const hashes = new Array(core.CIRCUIT_DEPTH + 1);
  ...
  for (let level = 1; level <= core.CIRCUIT_DEPTH; level++) {
...
async function buildPathFromLeaves(leaves, leafIndex) {
  ...
  for (let level = 0; level < core.CIRCUIT_DEPTH; level++) {
  ...
  const rootLevel = levels[core.CIRCUIT_DEPTH];
  const defaultRoot = zeroHashes[core.CIRCUIT_DEPTH];
...
var DEFAULT_NETWORK_PASSPHRASE = stellarSdk.Networks.TESTNET;
var BASE_FEE = "100000";
...
function hexToScBytes(hexStr, expectedBytes) {
  ...
  return stellarSdk.xdr.ScVal.scvBytes(buffer.Buffer.from(hexStr, "hex"));
}
function validateGroth16Proof(proof) {
  ...
  if (proof.publicSignals.length !== core.N_PUBLIC) {
    errors.push(
      `publicSignals: expected ${core.N_PUBLIC} elements, received ${proof.publicSignals.length}`
    );
  } else {
...
async function resolveSender() {
  ...
  return {
    address: stellarSdk.Keypair.fromSecret(secret).publicKey(),
    isBrowser: false
  };
}
async function signAndSubmit(preparedTx, isBrowser, networkPassphrase, rpcServer, rpcUrl) {
  ...
  if (isBrowser) {
    ...
    signedTx = stellarSdk.TransactionBuilder.fromXDR(signResult.signedTxXdr, networkPassphrase);
  } else {
    const secret = process.env["SENDER_SECRET"];
...
async function payWithZkGroth16(merchant, amount, proof, config, resolvedSender) {
  ...
  const publicInputsScVal = stellarSdk.xdr.ScVal.scvVec(
  ...
  const tx = new stellarSdk.TransactionBuilder(
    new stellarSdk.Account(senderAddress, accountResponse.sequenceNumber()),
...
exports.RootSyncError = RootSyncError;
...
exports.executeFluppyPayment = executeFluppyPayment;
exports.generateSecret = generateSecret;
exports.generateZkProof = generateZkProof;
exports.getContractMerkleRoot = getContractMerkleRoot;
